...
import { X, ExternalLink, Download, Award, ChevronLeft, ChevronRight, CheckCircle2, ShieldCheck } from 'lucide-react';
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 bg-slate-950/85 backdrop-blur-xl"
      >
        <motion.div
          ...
          className="w-full max-w-6xl h-[92vh] flex flex-col rounded-3xl bg-slate-900 border border-slate-700/60 shadow-2xl overflow-hidden relative"
        >
          {/* Header Bar */}
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-base sm:text-lg font-bold text-white truncate max-w-[280px] sm:max-w-[450px]">
                    {achievement.title}
                  </h3>
                  <span
                    className="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider border shrink-0"
                    style={{
          ...
          {/* Main Body */}
          <div className="flex-1 grid lg:grid-cols-3 overflow-hidden bg-slate-950">
            {/* Left/Center: Visual Preview Box */}
            <div className="lg:col-span-2 p-4 md:p-8 flex items-center justify-center bg-slate-950/90 relative overflow-y-auto">
              <div className="relative max-w-full max-h-full flex items-center justify-center">
                {!imageError && achievement.previewImage ? (
                  <img
                    src={achievement.previewImage}
                    alt={achievement.title}
                    className="max-w-full max-h-[72vh] object-contain rounded-2xl border border-slate-800 shadow-2xl transition-transform hover:scale-[1.01]"
            ...
            <div className="p-6 md:p-8 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-800 overflow-y-auto flex flex-col justify-between space-y-6">
              ...
              <div className="pt-6 border-t border-slate-800 space-y-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-primary via-blue-600 to-accent text-white font-bold text-sm shadow-xl shadow-primary/20 flex items-center justify-center gap-2 hover:opacity-95 transition-opacity"
                >
                ...
                <a
                  href={achievement.fileUrl}
                  download
                  className="w-full py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 flex items-center justify-center gap-2 transition-colors"
                >
