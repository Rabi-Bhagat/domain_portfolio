      ...
      >
        <AnimatePresence>
          {filteredProjects.map((project) => (
            ...
            >
              <TiltCard3D className="glass-card glass-card-hover p-6 flex flex-col group relative overflow-hidden h-full">
                 {/* Hover Glow Effect */}
                ...
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                        {project.tech.split(", ").map((tech) => (
                            <span key={tech} className="px-3 py-1 bg-slate-100 dark:bg-slate-800/50 rounded-full text-xs font-medium text-emerald-700 dark:text-emerald-300 border border-emerald-500/20">
                                {tech}
                            </span>
                        ))}
                    </div>

                    <div className="flex gap-4 mt-auto pt-4 border-t border-slate-200 dark:border-white/5">
                        <a
                          href={project.repo}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white transition-colors hover:underline decoration-slate-500 underline-offset-4 pointer-events-auto"
                        >
                          <Github size={18} /> Code
                        </a>
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 text-sm font-semibold text-primary hover:text-blue-600 dark:hover:text-blue-300 transition-colors ml-auto group/link pointer-events-auto"
                        >
